...
async function main() {
  ...
  const cat1 = await prisma.inventoryCategory.upsert({
    ...
    create: { name: 'Thiết bị hoàn thiện', code: 'TBHT', parentId: root.id }
  });
  ...
  // 4. Update existing MaterialItems to use these new categories
  const items = [
    { code: 'BT-001', name: 'Băng tan (Cao su non)', catCode: 'KCT', unit: 'Cuộn', spec: '10m/cuộn', material: 'Nhựa' },
    { code: 'RS-21', name: 'Ron cao su phi 21', catCode: 'VTSX', unit: 'Cái', spec: 'Phi 21', material: 'Cao su đen' }, // Just example
  ...
  console.log('Seeding materials into shared category table completed');
}
