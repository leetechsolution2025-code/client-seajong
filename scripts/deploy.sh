...
if pm2 list | grep -q "${APP_NAME}"; then
    ...
    log "App đã reload (zero-downtime)"
else
    pm2 start ecosystem.config.js
    pm2 startup 2>/dev/null | grep "sudo" | bash || true
    pm2 save
    log "App đã khởi động lần đầu"
fi
...
NGINX
        ln -sf "${NGINX_CONF}" /etc/nginx/sites-enabled/seajong 2>/dev/null || true
...
echo -e "  admin@seajong.vn  / Admin@123"
echo ""
