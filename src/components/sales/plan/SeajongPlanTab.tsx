...
const DEFAULT_PLAN_ROWS: PlanItemRow[] = [
  ...
  { stt: "2.1", item: "Chi phí mở đại lý", target: 1050000000, ratio: 8.1, note: "Tổng chi phí mở mới đại lý" },
  { stt: "2.1.1", item: "Chi phí quầy kệ", target: 500000000, ratio: 3.8, note: "Đóng mới & nâng cấp quầy kệ trưng bày" },
  { stt: "2.1.2", item: "Chi phí hoa hồng", target: 400000000, ratio: 3.1, note: "Chiết khấu thương mại và hoa hồng đại lý" },
  { stt: "2.1.3", item: "POSM mở đại lý", target: 100000000, ratio: 0.8, note: "Tài liệu bán hàng, quà tặng khai trương" },
  { stt: "2.1.4", item: "Hỗ trợ giám sát lắp đặt", target: 50000000, ratio: 0.4, note: "Chi phí hỗ trợ kỹ thuật tại quầy kệ" },
  { stt: "2.2", item: "Lương nhân viên kinh doanh", target: 600000000, ratio: 4.6, note: "Lương cơ bản đội ngũ kinh doanh" },
  { stt: "2.3", item: "Thưởng", target: 300000000, ratio: 2.3, note: "Thưởng đạt KPI tháng/quý/năm" },
  { stt: "2.4", item: "Công tác phí và tiếp khách", target: 150000000, ratio: 1.2, note: "Chi phí công tác, di chuyển thị trường" },
  { stt: "2.5", item: "Tiếp khách và hội thảo", target: 50000000, ratio: 0.4, note: "Hội nghị khách hàng thường niên" },
  { stt: "2.6", item: "Khuyến mãi", target: 600000000, ratio: 4.6, note: "Quà tặng khuyến mại, giảm giá trực tiếp" }
];
  ...
  for (let m = 1; m <= 12; m++) {
    initial[m] = {
      ...
      revenueRows: [
        ...
        { stt: "1.1", item: "Doanh thu từ đại lý lắp kệ", value: 0 },
        { stt: "1.2", item: "Doanh thu từ đại lý chính thức", value: 0 },
        { stt: "1.3", item: "Doanh thu từ đại lý lấy hàng", value: 0 },
        { stt: "voriger", item: "VORIGER", value: 0, isHeader: true },
        { stt: "2.1", item: "Doanh thu từ Shopee", value: 0 },
        { stt: "2.2", item: "Doanh thu từ B2B", value: 0 },
        { stt: "2.3", item: "Doanh thu từ khách ngoài", value: 0 },
      ],
    };
  }
  return initial;
...
const recalculatePlanRowsWithStaffTotal = (
  ...
  // Tính tổng doanh thu (các mục con bắt đầu bằng "1." và chỉ có 1 dấu chấm)
  const revenueSubItems = updated.filter(r => r.stt && r.stt.toString().startsWith("1.") && r.stt.toString().split(".").length === 2);
  const totalRevenue = revenueSubItems.reduce((sum, r) => sum + (r.target || 0), 0);

  // Tính tổng chi phí (các mục con bắt đầu bằng "2." và chỉ có 1 dấu chấm)
  const costSubItems = updated.filter(r => r.stt && r.stt.toString().startsWith("2.") && r.stt.toString().split(".").length === 2);
...
export function SeajongPlanTab({ currentStep }: { currentStep: number }) {
  ...
  const applyMasterDataToRows = (
    basePlanRows: PlanItemRow[],
    baseStaffRows: StaffItemRow[],
    parsed: any,
    finalCostSales: number
  ) => {
    const mappedPlanRows = basePlanRows.map((row: PlanItemRow) => {
      let cleanRow = { ...row };
      ...
      else if (cleanRow.stt === "2.1.1") cleanRow.target = 0;
      else if (cleanRow.stt === "2.1.2") cleanRow.target = 0;
      else if (cleanRow.stt === "2.1.3") cleanRow.target = 0;
      else if (cleanRow.stt === "2.1.4") cleanRow.target = 0;
      else if (cleanRow.stt === "2.3") cleanRow.target = Number(parsed.c_biz_bonus) || 0;
      else if (cleanRow.stt === "2.4") cleanRow.target = Number(parsed.c_biz_travel) || 0;
      else if (cleanRow.stt === "2.5") cleanRow.target = 0;
      else if (cleanRow.stt === "2.6") {
        const promoCost = parsed.bizCosts?.find((c: any) => c.label?.toLowerCase().includes("khuyến mãi") || c.label?.toLowerCase().includes("km"));
        cleanRow.target = promoCost ? (Number(promoCost.val) || 0) : (Number(parsed.c_biz_promo) || 0);
      }
      cleanRow.masterTarget = cleanRow.target;

      return cleanRow;
    });

    const bizStaffList = parsed.bizStaff || [];
    let mappedStaffRows: StaffItemRow[] = bizStaffList.map((s: any, idx: number) => {
      const quantity = Number(s.qty) || 0;
      const basicSalary = Number(s.salary) || 0;
      
      const existingRow = baseStaffRows.find((r: StaffItemRow) => r.role === s.label);
      const performanceSalary = existingRow ? (existingRow.performanceSalary || 0) : 0;
      const allowance = existingRow ? (existingRow.allowance || 0) : 0;
      
      const totalBudget = (basicSalary + performanceSalary + allowance) * quantity * 12;
      return {
        stt: String(idx + 1),
        role: s.label || "Vị trí mới",
        basicSalary,
        performanceSalary,
        allowance,
        quantity,
    ...
    const customRows = baseStaffRows.filter((r: StaffItemRow) => {
      if (r.isTotal || r.isFullWidth) return false;
      if (bizStaffList.some((s: any) => s.label === r.role)) return false;
      
      const isDefault = DEFAULT_STAFF_ROWS.some(dr => dr.role === r.role);
    ...
    customRows.forEach((cr, i) => {
      mappedStaffRows.push({
        ...cr,
        stt: String(bizStaffList.length + i + 1)
      });
    });

    const totalQty = mappedStaffRows.reduce((sum, r) => sum + (r.quantity || 0), 0);
    const totalBgt = mappedStaffRows.reduce((sum, r) => sum + (r.totalBudget || 0), 0);

    mappedStaffRows.push({
      stt: "Tổng",
      role: "Tổng quỹ lương nhân sự",
      basicSalary: 0,
      performanceSalary: 0,
      allowance: 0,
      quantity: totalQty,
    ...
    return { finalPlanRows, mappedStaffRows };
  };
  ...
  const mapMasterDataToStates = (parsed: any, finalCostSales: number) => {
    ...
    const { finalPlanRows, mappedStaffRows } = applyMasterDataToRows(DEFAULT_PLAN_ROWS, DEFAULT_STAFF_ROWS, parsed, finalCostSales);
    ...
    const fetchPlan = async () => {
      ...
      try {
        ...
        if (res.ok) {
          ...
          if (masterPlanObj) {
            const { finalPlanRows, mappedStaffRows } = applyMasterDataToRows(currentPlanRows, cleanStaffRows, masterPlanObj, finalCostSales);
            setPlanRows(finalPlanRows);
            setStaffRows(mappedStaffRows);
          } else {
          ...
          if (data.monthlyTargets) {
            ...
            for (let m = 1; m <= 12; m++) {
              ...
              if (!healedTargets[m].revenueRows || healedTargets[m].revenueRows.length === 0) {
                ...
                healedTargets[m].revenueRows.forEach((row: any) => {
                  if (row.stt === "1.1") row.item = "Doanh thu từ đại lý lắp kệ";
                  if (row.stt === "1.2") row.item = "Doanh thu từ đại lý chính thức";
                  if (row.stt === "1.3") row.item = "Doanh thu từ đại lý lấy hàng";
                  if (row.stt === "2.1") row.item = "Doanh thu từ Shopee";
                  if (row.stt === "2.2") row.item = "Doanh thu từ B2B";
                  if (row.stt === "2.3") row.item = "Doanh thu từ khách ngoài";
                });
              }
            }
  ...
  const columns: TableColumn<PlanItemRow>[] = [
    {
      ...
      align: "center",
      render: (row) => {
        const isChildOfChild = row.stt && row.stt.toString().split(".").length === 3;
        return (
          <span className={`fw-bold ${row.isTotal ? "text-dark" : "text-secondary"}`}>
            {isChildOfChild ? "-" : row.stt}
          </span>
        );
      }
    },
        ...
        return (
          <div className="d-flex flex-column gap-0.5">
            <input
              ...
              style={{
 fontSize: 13, pointerEvents: (!isEditMode || isTotal) ? "none" : "auto", height: "20px" }}
            />
          </div>
        );
      }
    },
    {
      ...
      render: (row, idx) => {
        const isTotal = row.isTotal;
        const isRow22 = row.stt === "2.2";
        const isTotalCost = row.stt === "2";
        const isOverBudget = isTotalCost && masterCostSales > 0 && (row.target || 0) > masterCostSales;
        const isChildOverBudget = !isTotalCost && row.masterTarget !== undefined && (row.target || 0) > row.masterTarget;
        ...
        return (
          <input
            ...
            onChange={(e) => !isTotal && !isRow22 && handleCellChange(idx, "target", e.target.value)}
            onKeyDown={(e) => handleTableInputKeyDown(e, "plan-target")}
            readOnly={!isEditMode || isTotal || isRow22}
            className={`form-control form-control-sm border-0 bg-transparent px-0 text-end ${isChildOverBudget ? "text-danger fw-bold" : "text-dark fw-medium"}`}

            style={{
              fontSize: 13,
              pointerEvents: (!isEditMode || isTotal || isRow22) ? "none" : "auto",
              backgroundColor: isRow22 && isEditMode ? "rgba(108, 117, 125, 0.08)" : undefined,
              borderRadius: isRow22 && isEditMode ? "4px" : undefined,
              paddingRight: isRow22 && isEditMode ? "4px" : undefined
            }}
          />
        );
      }
    ...
    ...(isEditMode ? [{
      ...
      render: (row: PlanItemRow, idx: number) => {
        ...
        const isDefaultRow = DEFAULT_PLAN_ROWS.some(r => r.stt === row.stt);
        if (!isChildRow || isDefaultRow) return null;
              ...
              <div>
                ...
                <div className="d-flex flex-column gap-2" style={{ paddingLeft: "32px" }}>
                  <div className="d-flex justify-content-between align-items-center" style={{ fontSize: 12.5 }}>
                    ...
                    <span className="text-dark">{getYearlyTargetSum("chinhThuc")} đại lý</span>
                  </div>
                  <div className="d-flex justify-content-between align-items-center" style={{ fontSize: 12.5 }}>
              ...
              <div className="mt-2">
                ...
                <div style={{ paddingLeft: "32px" }}>
                  <div className="row g-4">
                    {(() => {
                      const rows = monthlyTargets[selectedMonth]?.revenueRows || [];
                      const seajongItems = rows.slice(1, 4); // 1.1, 1.2, 1.3
                      const vorigerItems = rows.slice(5, 8); // 2.1, 2.2, 2.3
                      const seajongTotal = seajongItems.reduce((acc, row) => acc + (row.value || 0), 0);
                      const vorigerTotal = vorigerItems.reduce((acc, row) => acc + (row.value || 0), 0);

                      const renderGroup = (title: string, items: typeof rows, total: number, startIndex: number) => (
                        <div className="col-12 col-xl-6">
                          <div className="card shadow-sm border-0 bg-white" style={{ borderRadius: "12px", border: "1px solid rgba(0, 0, 0, 0.08) !important" }}>
                            <div className="card-body p-4">
                              <div className="rounded-3 p-3 mb-4" style={{ backgroundColor: "rgba(59, 130, 246, 0.08)", border: "1px solid rgba(59, 130, 246, 0.15)" }}>
                                <div className="fw-bold text-primary mb-1 text-uppercase" style={{ fontSize: 11, letterSpacing: "0.5px" }}>TỔNG DOANH THU {title}</div>
                                <div className="fw-bold text-primary d-flex align-items-baseline gap-1">
                                  <span style={{ fontSize: 24 }}>{total.toLocaleString("vi-VN")}</span>
                                  <span style={{ fontSize: 14 }}>đ</span>
                                </div>
                              </div>
                              <div className="d-flex flex-column gap-3">
                                {items.map((row, idx) => {
                                  ...
                                  return (
                                    <div key={row.stt}>
                                      <label className="fw-semibold text-secondary mb-1" style={{ fontSize: 13 }}>{row.item}</label>
                                      <div className="input-group">
                                        <input
                                          type="text"
                                          className="form-control fw-semibold revenue-input"
                                          value={row.value ? row.value.toLocaleString("vi-VN") : ""}
                                          onChange={(e) => handleMonthlyRevenueChange(selectedMonth, realIdx, e.target.value)}
                                          placeholder="0"
                                          style={{ 
                                            fontSize: 13.5, 
                                            height: "34px", 
                                          ...
                                          onKeyDown={(e) => {
                                            if (e.key === "Enter") {
                                              e.preventDefault();
                                              const inputs = Array.from(document.querySelectorAll(".revenue-input")) as HTMLInputElement[];
                                        ...
                                        />
                                        <span className="input-group-text fw-semibold text-dark bg-white" style={{ width: "48px", justifyContent: "center", borderColor: "rgba(59, 130, 246, 0.15)", borderLeft: "none", fontSize: 13 }}>đ</span>
                                      </div>
                                    </div>
                                  );
                                })}
                      ...
                      return (
                        <>
                          {renderGroup("SEAJONG", seajongItems, seajongTotal, 1)}
                          {renderGroup("VORIGER", vorigerItems, vorigerTotal, 5)}
                        </>
                      );
                    })()}
                  </div>
