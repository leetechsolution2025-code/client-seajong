...
interface Props {
  isOpen: boolean;
  onClose: () => void;
  client: { id: string; name: string; shortName: string; config: string | null } | null;
...
export function ExportModal({ isOpen, onClose, client, onSaved }: Props) {
  ...
  useEffect(() => {
    ...
    fetch("/api/admin/departments")
      ...
      .then((data: Department[]) => {
        ...
        // Ưu tiên load từ config đã lưu
        if (client?.config) {
          try {
            const savedConfig = JSON.parse(client.config);
            if (savedConfig.selectedModules && Array.isArray(savedConfig.selectedModules)) {
              setSelected(savedConfig.selectedModules);
              return;
            }
          } catch (e) {
        ...
        // Mặc định chọn nhóm core nếu chưa có config
  ...
  async function handleExport() {
    ...
    try {
      ...
      if (data.success) {
        // Lưu lại cấu hình modules đã chọn vào DB để lần sau không phải chọn lại
        const newConfig = JSON.stringify({ selectedModules: selected });
        await fetch(`/api/admin/clients/${client.id}`, {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ config: newConfig }),
        }).then(r => r.json()).then(d => {
          if (d.success && onSaved) onSaved(d.client);
        }).catch(err => console.error("Failed to save client config", err));
