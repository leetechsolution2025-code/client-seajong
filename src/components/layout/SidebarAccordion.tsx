...
export function SidebarAccordion({ overviewHref, groups, isCollapsed, onMenuSelect, userRole, userLevelOrder, onRequestRecruitment, onRequestTraining }: Props) {
  ...
  return (
    <nav className="d-flex flex-column flex-grow-1 overflow-x-hidden px-2 py-2" style={{ overflowY: "auto" }}>
      ...
      {/* ── Quản trị hệ thống (admin group) nếu tồn tại ── */}
      {adminGroup && renderGroup(adminGroup)}
      
      {/* ── Lưu trữ (Thư viện tài liệu & Cuộc họp) ── */}
      {(() => {
        const isLuuTruActive = pathname.startsWith("/my/documents") || pathname.startsWith("/my/meetings");
        const isOpen = openGroup === "luutru";
        
        return (
          <div className="mb-1">
            <button
              onClick={() => { if (!isCollapsed) setOpenGroup(isOpen ? null : "luutru"); }}
              className={`si-btn w-100 d-flex align-items-center gap-2 px-3 py-2 border-0${isLuuTruActive ? " active" : ""}`}
              style={{ cursor: "pointer" }}
            >
              <i className="bi bi-archive flex-shrink-0" style={{ fontSize: 16 }} />
              <AnimatePresence>
                {!isCollapsed && (
                  <motion.span
                    initial={{ opacity: 0, width: 0 }} animate={{ opacity: 1, width: "auto" }}
              ...
              {isOpen && !isCollapsed && (
                ...
                >
                  <div className="ps-3 pe-1 pb-1">
                    {/* Thư viện tài liệu */}
                    {(() => {
                      const active = pathname.startsWith("/my/documents");
                      return (
                        <Link
                          href="/my/documents"
                          onClick={onMenuSelect}
                    ...
                    {/* Nội dung cuộc họp */}
                    {(() => {
                      const active = pathname.startsWith("/my/meetings");
                      return (
                        <Link
                          href="/my/meetings"
                          onClick={onMenuSelect}
      ...
      {(() => {
        ...
        if (isLogistics || isMarketing || isProduction || isFinance) return null; // Ẩn chức năng Kho hàng trong bộ phận kho, marketing, sản xuất, tài chính
      ...
      <div className="mt-auto pt-2">
        ...
        {[
          ...
          { name: "Yêu cầu cá nhân",     href: "/my/leave-request", icon: "bi-file-earmark-plus" },
        ].map((item) => {
