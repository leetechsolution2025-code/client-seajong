...
export function Topbar({ onToggleSidebar }: TopbarProps) {
  ...
  useEffect(() => {
    ...
    if (empId) {
      fetch(`/api/hr/employees/${empId}`)
        .then(r => r.json())
        .then(d => {
          if (d?.employee?.avatarUrl) setCurrentUserAvatar(d.employee.avatarUrl);
        })
        .catch(() => {});
    }
  }, [session]);

  /* ── Xử lý URL params để mở Approval Center ── */
  useEffect(() => {
    const aid = searchParams.get("approvalId");
    const eid = searchParams.get("entityId");
    if (aid || eid) {
      setTargetApprovalId(aid);
      setTargetEntityId(eid);
      setApprovalOpen(true);
      // Xóa params sau khi đã nhận diện (tùy chọn, để URL sạch hơn)
  ...
  const userImage         = (session?.user as any)?.image || currentUserAvatar || null;
  const displayClientName = company?.name || session?.user?.clientName || "EOS";
  const clientLogoUrl     = normalizeImgSrc(company?.logoUrl || (session?.user as any)?.clientLogoUrl) || null;
  ...
  return (
    <header className="app-topbar" style={{
      ...
      <NotificationOffcanvas
        ...
        userRole={userRole}
        onOpenApproval={(aid, eid) => {
          setTargetApprovalId(aid || null);
          setTargetEntityId(eid || null);
          setApprovalOpen(true);
          setNotifOpen(false);
        }}
      />
