...
function getMota(l: NhapLine): string {
  if (l.type === "nhap")        return "Nhập kho hàng hoá" + (l.fromKho ? ` từ ${l.fromKho}` : "");
  if (l.type === "luan-chuyen" || l.type === "chuyen")
    return `Luân chuyển kho${l.fromKho ? ` từ ${l.fromKho}` : ""}`;
  if (l.type === "dieu-chinh")  return "Điều chỉnh tăng tồn kho";
  return l.lyDo ?? l.type;
}

// ── Component ─────────────────────────────────────────────────────────────────
export function BangKeNhapKhoModal({ warehouseId, warehouseName, onClose }: Props) {
  ...
  const [lines,   setLines]         = React.useState<NhapLine[]>([]);
  ...
  const load = React.useCallback(() => {
    setLoading(true);
    fetch(`/api/plan-finance/reports/bang-ke-nhap?warehouseId=${warehouseId}&from=${from}&to=${to}`)
    ...
    <div style={{ fontFamily: "'Roboto Condensed','Arial Narrow',Arial,sans-serif", fontSize: 13, color: "#000", lineHeight: 1.4 }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 8 }}>
        ...
        <div style={{ textAlign: "right", fontSize: 11, flexShrink: 0 }}>
          <p style={{ margin: 0, fontWeight: 700 }}>Mẫu số S10-DN</p>
      ...
      <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 11.5, marginBottom: 8 }}>
        <thead>
          <tr>
            ...
            <th rowSpan={2} style={thCss(110)}>Số phiếu nhập</th>
            ...
            <th rowSpan={2} style={thCss(80)}>Đơn giá nhập kho</th>
        ...
        <tbody>
          {loading ? (
            ...
            <tr><td colSpan={10} style={{ border: B1, padding: 20, textAlign: "center", color: "#94a3b8" }}>Không có dữ liệu nhập kho trong kỳ</td></tr>
          ) : lines.map((l, i) => (
  ...
  const actions = (
    <button
      onClick={() => printDocumentById("bk-nhap-doc", "landscape", `Bảng kê nhập kho - ${warehouseName}`)}
      style={{ padding: "8px 22px", border: "none", background: "#10b981", color: "#fff", borderRadius: 8, fontSize: 13, fontWeight: 700, cursor: "pointer", display: "flex", alignItems: "center", gap: 7 }}>
