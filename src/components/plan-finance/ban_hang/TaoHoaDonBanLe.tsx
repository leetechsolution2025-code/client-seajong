...
export function TaoHoaDonBanLe({ open, onClose, onSaved }: Props) {
  ...
  useEffect(() => {
    ...
    searchTimer.current = setTimeout(() => {
      setSearching(true);
      fetch(`/api/logistics/inventory?search=${encodeURIComponent(productSearch)}&limit=30&includeManufactured=true`)
        .then(r => r.json())
  ...
  useEffect(() => {
    ...
    _lineId = 1;
    setSearching(true);
    fetch("/api/logistics/inventory?limit=30&includeManufactured=true")
      .then(r => r.json())
      .then((d: any) => setInventoryResults(d.items || []))
      .catch(() => {})
      .finally(() => setSearching(false));
