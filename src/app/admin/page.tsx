...
// ── Section components (module-level — không định nghĩa bên trong page) ────────
...
export default function AdminPage() {
  ...
  // Fetch từ DB, không dùng session/cache
  useEffect(() => {
    fetch("/api/company", { cache: "no-store" })
      .then(r => r.json())
      .then(data => {
        if (data && !data.error) {
          setForm({ name: data.name ?? "", shortName: data.shortName ?? "", slogan: data.slogan ?? "",
            address: data.address ?? "", phone: data.phone ?? "", email: data.email ?? "",
            website: data.website ?? "", taxCode: data.taxCode ?? "", legalRep: data.legalRep ?? "",
  ...
  // Upload ảnh lên server, lưu đường dẫn vào form
  ...
  return (
    ...
    >
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 16 }}>
        <div>
          ...
          <p style={{ margin: "4px 0 0", fontSize: 13, color: "var(--muted-foreground)" }}>
            Cập nhật thông tin tổ chức hiển thị trong hệ thống
          </p>
        </div>
        <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 6 }}>
          {error && (
            <p style={{ margin: 0, fontSize: 12, color: "#f43f5e", textAlign: "right", maxWidth: 260 }}>{error}</p>
          )}
          {success && (
            <p style={{ margin: 0, fontSize: 12, color: "#10b981", fontWeight: 600 }}>
              <i className="bi bi-check2-circle" /> Đã lưu thành công
            </p>
          )}
          <button onClick={handleSave} disabled={saving || loading}
            style={{
              padding: "9px 22px", border: "none", borderRadius: "var(--radius)",
              background: saving ? "var(--muted)" : "var(--primary)", color: saving ? "var(--muted-foreground)" : "#fff",
      ...
      {loading ? (
        <div style={{ textAlign: "center", padding: "60px 0", color: "var(--muted-foreground)", fontSize: 13 }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              <div style={{ display: "grid", gridTemplateColumns: "3fr 1fr", gap: 16 }}>
                <Field label="Tên công ty" required>
                  <Input value={form.name} onChange={set("name")} placeholder="Công ty TNHH Leetech" />
                </Field>
              ...
              <Field label="Khẩu hiệu (Slogan)">
                <textarea
                  rows={2} value={form.slogan}
                  onChange={e => setForm(f => ({ ...f, slogan: e.target.value }))}
                  placeholder="Hệ điều hành doanh nghiệp..."
              ...
              <Field label="Địa chỉ">
                <Input value={form.address} onChange={set("address")} placeholder="Số nhà, đường, quận/huyện, tỉnh/thành phố" />
              </Field>
            </div>
          </Card>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr auto", gap: 10, alignItems: "flex-end" }}>
                <Field label="Đường dẫn Logo">
                  <Input value={form.logoUrl} onChange={set("logoUrl")} placeholder="/uploads/logo.png hoặc https://..." />
                </Field>
                <div>
                  <Label text="Upload ảnh" />
                  <label style={{
                    display: "inline-flex", alignItems: "center", gap: 6, padding: "8px 14px",
                    border: `1px solid ${uploading ? "var(--primary)" : "var(--border)"}`,
                    borderRadius: "var(--radius)", cursor: uploading ? "not-allowed" : "pointer",
                    fontSize: 13, fontWeight: 600, color: uploading ? "var(--primary)" : "var(--foreground)",
                    background: "var(--muted)", transition: "all 0.15s", whiteSpace: "nowrap",
                    opacity: uploading ? 0.7 : 1,
                  }}>
                    <i className={`bi ${uploading ? "bi-arrow-repeat" : "bi-upload"}`}
                      style={{ fontSize: 13, animation: uploading ? "spin 1s linear infinite" : "none" }} />
              ...
              {form.logoUrl && (
                <div style={{ display: "flex", alignItems: "center", gap: 14, padding: "12px 14px",
                  background: "var(--muted)", borderRadius: 10, border: "1px solid var(--border)" }}>
                  <img src={form.logoUrl} alt="Logo preview"
                    style={{ width: 60, height: 60, objectFit: "contain", borderRadius: 8,
                      border: "1px solid var(--border)", background: "#fff", padding: 4 }}
                  ...
                  <button onClick={() => setForm(f => ({ ...f, logoUrl: "" }))}
                    style={{ padding: "4px 8px", border: "1px solid var(--border)", background: "transparent",
                      color: "var(--muted-foreground)", fontSize: 11, cursor: "pointer", borderRadius: "var(--radius)" }}
                  >
