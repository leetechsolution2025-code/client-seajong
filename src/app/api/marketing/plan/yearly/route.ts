...
export async function GET(request: Request) {
  try {
    ...
    // Fetch approval statuses for monthly executions
    const approvalRequests = await prisma.approvalRequest.findMany({
      where: {
        entityType: "marketing_monthly_execution",
        entityId: {
          contains: "-" // Fetch all and filter in JS for robustness across multiple plans
        }
      },
      include: {
        comments: {
    ...
    const plans = rawPlans.map((p: any) => {
      ...
      // Reconstruct monthlyPlans JSON from executionMonths
      const reconstructedMonthlyPlans: any = {};
      (p.executionMonths || []).forEach((em: any) => {
        if (!reconstructedMonthlyPlans[em.taskId]) reconstructedMonthlyPlans[em.taskId] = {};
        
        const items: any[] = [];
        (em.groups || []).forEach((group: any) => {
          // Add Header item
          items.push({
            id: group.id,
            name: group.name,
          ...
          // Add Detail items
          (group.tasks || []).forEach((detail: any) => {
            items.push({
              id: detail.id,
              name: detail.name,
              time: detail.week,
        ...
        reconstructedMonthlyPlans[em.taskId][em.month] = items;
      });

      // Fetch monthly statuses for this plan
      const monthlyStatuses: any = {};
      const planApprovals = approvalRequests.filter(req => req.entityId.startsWith(`${p.id}-`));
      planApprovals.forEach(req => {
        // entityId: `${selectedPlanId}-${month}-${taskName}`
        const parts = req.entityId.split('-');
        if (parts.length >= 3) {
          const monthStr = parts[parts.length - 2].replace('m', '');
          const monthNum = parseInt(monthStr);
          const taskName = parts[parts.length - 1].toUpperCase();
          if (!monthlyStatuses[taskName]) monthlyStatuses[taskName] = {};
          
          let displayStatus = "Bản nháp";
          const s = req.status.toUpperCase();
          if (s === "PENDING") displayStatus = "Chờ duyệt";
          else if (s === "APPROVED") displayStatus = "Đã duyệt";
          else if (s === "REJECTED") displayStatus = "Từ chối";
          
          monthlyStatuses[taskName][monthNum] = {
            status: displayStatus,
            requestId: req.id,
            commentCount: (req as any).comments?.length || 0
          };
        }
      });
      ...
      return {
        ...
        monthlyPlans: reconstructedMonthlyPlans,
        monthlyExecutionStatuses: monthlyStatuses
      };
    });
