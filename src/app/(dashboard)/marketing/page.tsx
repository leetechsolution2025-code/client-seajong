...
function CreateCampaignOffcanvas({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return (
    <>
      ...
      <div className="mkt-offcanvas-width" style={{
...
export default function MarketingPage() {
  ...
  const [activeFeedIndex, setActiveFeedIndex] = useState(-1);
  ...
  const handleImportExcel = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (evt) => {
      try {
        ...
        const campaignNameIdx = findIdx(["Tên chiến dịch", "Campaign Name", "campaign_name"]);
        const campaignIdIdx = findIdx(["ID chiến dịch", "Campaign ID", "campaign_id"]);
        const platformIdx = findIdx(["Nền tảng", "Platform", "platform", "Kênh"]);
        const dateIdx = findIdx(["Ngày", "Date", "date_start"]);
        const spendIdx = findIdx(["Chi phí", "Spend", "spend", "Tổng chi"]);
        const impressionsIdx = findIdx(["Hiển thị", "Impressions", "impressions", "Lượt hiển thị"]);
        const clicksIdx = findIdx(["Click", "Clicks", "clicks", "Lượt click"]);
        const leadsIdx = findIdx(["Lead", "Leads", "leads", "Số lead"]);

        if (campaignNameIdx === -1 || dateIdx === -1 || spendIdx === -1) {
          alert("File Excel thiếu cột bắt buộc: Tên chiến dịch, Ngày, Chi phí");
          return;
        }
        ...
        for (let i = 1; i < rawData.length; i++) {
          const row = rawData[i];
          if (!row[campaignNameIdx]) continue;
          ...
          items.push({
            campaignName: String(row[campaignNameIdx]).trim(),
            campaignExternalId: campaignIdIdx !== -1 && row[campaignIdIdx] ? String(row[campaignIdIdx]).trim() : null,
            platform: platformIdx !== -1 && row[platformIdx] ? String(row[platformIdx]).trim().toLowerCase() : "facebook",
  ...
  const confirmImport = async () => {
    setIsImporting(true);
    try {
      const res = await fetch("/api/marketing/dashboard/import", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ items: importData, mode: importMode })
      });
      if (res.ok) {
        alert(`Đã import thành công dữ liệu!`);
        setImportModalOpen(false);
        fetchDashboard();
        fetchLeads();
      } else {
        const err = await res.json();
        alert("Lỗi import: " + (err.error ?? "Lỗi không xác định"));
      }
    } catch (e: any) {
  ...
  useEffect(() => {
    ...
    targetCamps.forEach(camp => {
      const days = camp.insights?.data || [];
      days.forEach((d: any) => {
        const dateStr = d.date_start;
        if (!aggregatedDays[dateStr]) {
          aggregatedDays[dateStr] = {
            date_start: dateStr,
            reach: 0,
            clicks: 0,
            likes: 0,
        ...
        aggregatedDays[dateStr].reach += parseInt(d.reach || "0");
        aggregatedDays[dateStr].clicks += parseInt(d.clicks || "0");
        aggregatedDays[dateStr].likes += parseInt(d.likes || "0");
        
        const lAct = d.actions?.find((a: any) => a.action_type === "lead");
        aggregatedDays[dateStr].leads += parseInt(lAct?.value || d.leads || "0");
      });
    });

    const sorted = Object.values(aggregatedDays).sort((a: any, b: any) => a.date_start.localeCompare(b.date_start));
    ...
    setLeadSeries([
      { name: "Lượt tiếp cận", data: dataSlice.map(d => d.reach) },
      { name: "Lượt tương tác (Clicks)", data: dataSlice.map(d => d.clicks) },
      { name: "Lượt thích (Likes)", data: dataSlice.map(d => d.likes) },
      { name: "Khách hàng tiềm năng (Leads)", data: dataSlice.map(d => d.leads) }
    ]);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", background: "var(--background)" }}>
      <style>{`
        ...
        .mkt-campaign-feed-row {
          display: flex;
          align-items: center;
          gap: 16px;
        }
        .mkt-campaign-metrics {
          display: flex;
          gap: 24px;
          flex-shrink: 0;
          text-align: right;
        ...
        @media (max-width: 640px) {
          ...
          .mkt-kpi-grid > div p:first-child {
            font-size: 16px !important;
          }
          .mkt-kpi-grid > div p:last-child {
          ...
          .mkt-campaign-metrics span:first-child {
      ...
      {/* TRẢ LẠI PAGE HEADER CHUẨN CỦA HỆ THỐNG - ĐÃ ĐƯỢC CHỈNH THEO TRANG TỔNG QUAN */}
      <PageHeader
        title="Marketing"
      ...
      <div className="mkt-page-body d-flex flex-column flex-grow-1" style={{ minHeight: 0 }}>
        ...
        >
          {/* LIVE METRICS & FEED */}
          <div className="mkt-metrics-wrapper" style={{ display: "flex", flexDirection: "column", gap: 24, flex: 1, minHeight: "100%" }}>
                  ...
                  <div style={{ marginTop: 16, paddingTop: 16, borderTop: "1px solid var(--border)" }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
                      ...
                      <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                        {(() => {
                           const activeCamps = fbCampaigns.filter(c => c.status?.toUpperCase() === "ACTIVE");
                           if (activeCamps.length <= 1) return null;
                           return (
                             <div style={{ display: "flex", alignItems: "center", gap: 6, marginRight: 6 }}>
                               <button
                                 onClick={() => setActiveFeedIndex(prev => Math.max(-1, prev - 1))}
                                 disabled={activeFeedIndex === -1}
                                 style={{ width: 22, height: 22, borderRadius: 6, border: "1px solid var(--border)", background: "var(--background)", color: activeFeedIndex === -1 ? "var(--muted-foreground)" : "var(--foreground)", cursor: activeFeedIndex === -1 ? "default" : "pointer", display: "flex", alignItems: "center", justifyContent: "center", transition: "all 0.2s" }}
                               ><i className="bi bi-chevron-left" style={{ fontSize: 10 }} /></button>
                               <span style={{ fontSize: 11, fontWeight: 700, color: "var(--muted-foreground)", width: 45, textAlign: "center" }}>
                                 {activeFeedIndex === -1 ? "Tất cả" : `${activeFeedIndex + 1}/${activeCamps.length}`}
                               </span>
                               <button
                                 onClick={() => setActiveFeedIndex(prev => Math.min(activeCamps.length - 1, prev + 1))}
                                 disabled={activeFeedIndex >= activeCamps.length - 1}
                                 style={{ width: 22, height: 22, borderRadius: 6, border: "1px solid var(--border)", background: "var(--background)", color: activeFeedIndex >= activeCamps.length - 1 ? "var(--muted-foreground)" : "var(--foreground)", cursor: activeFeedIndex >= activeCamps.length - 1 ? "default" : "pointer", display: "flex", alignItems: "center", justifyContent: "center", transition: "all 0.2s" }}
                      ...
                      {fbCampaignsLoading ? (
                        ...
                        if (activeFeedIndex === -1) {
                          ...
                          activeCamps.forEach(c => {
                            ...
                            totalSpend += parseFloat(todayData.spend || "0");
                            totalClicks += parseInt(todayData.clicks || "0");
                            totalImpressions += parseInt(todayData.impressions || "0");
                            totalLeads += parseInt(todayData.actions?.find((x: any) => x.action_type === "lead")?.value || "0");
                          });
                          const cpaValue = totalLeads > 0 ? Math.round(totalSpend / totalLeads) : 0;
                          const ctrValue = totalImpressions > 0 ? ((totalClicks / totalImpressions) * 100).toFixed(2) + "%" : "0%";
                              ...
                              >
                                <div style={{ flex: 1, minWidth: 200 }}>
                                  <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
                                    <i className="bi bi-megaphone-fill" style={{ color: "#10b981", fontSize: 16 }} />
                                    <span style={{ fontSize: 14, fontWeight: 800 }}>Tất cả {activeCamps.length} chiến dịch đang chạy</span>
                                  </div>
                                  <div style={{ fontSize: 11, color: "var(--muted-foreground)", display: "flex", gap: 16, flexWrap: "wrap" }}>
                                    <span style={{ display: "flex", alignItems: "center", gap: 4 }}><i className="bi bi-eye"></i> {totalImpressions.toLocaleString()} hiển thị</span>
                                    <span style={{ display: "flex", alignItems: "center", gap: 4 }}><i className="bi bi-cursor"></i> CTR: {ctrValue}</span>
                                    <span style={{ display: "flex", alignItems: "center", gap: 4 }}><i className="bi bi-wallet2"></i> CPA trung bình: {cpaValue > 0 ? `${cpaValue.toLocaleString('vi-VN')}đ` : "—"}</span>
                                  </div>
                                </div>
                                <div className="mkt-campaign-metrics">
                                  <div>
                                    ...
                                    <span style={{ fontSize: 15, fontWeight: 700, color: "var(--foreground)", fontFamily: "monospace" }}>{Math.round(totalSpend / 1000).toLocaleString()}k</span>
                                  </div>
                                  <div>
                                    <span style={{ display: "block", fontSize: 10, color: "var(--muted-foreground)", marginBottom: 2 }}>Clicks mới</span>
                                    <span style={{ fontSize: 15, fontWeight: 700, color: "var(--foreground)", fontFamily: "monospace" }}>{totalClicks.toLocaleString()}</span>
                                  </div>
                                  <div style={{ minWidth: 70 }}>
                                    ...
                                    <span style={{ fontSize: 16, fontWeight: 800, color: "#10b981", display: "flex", alignItems: "center", justifyContent: "flex-end", gap: 4 }}>
                                      <i className="bi bi-person-plus-fill" style={{ fontSize: 12 }} /> +{totalLeads}
                                    </span>
                                  </div>
                                </div>
                              </motion.div>
                            ...
                            >
                              <div style={{ flex: 1, minWidth: 200 }}>
                                ...
                                <div style={{ fontSize: 11, color: "var(--muted-foreground)", display: "flex", gap: 16, flexWrap: "wrap" }}>
                                  ...
                                  <span style={{ display: "flex", alignItems: "center", gap: 4 }}><i className="bi bi-eye"></i> {impressions.toLocaleString()} hiển thị</span>
                                ...
                                <tr key={c.id} style={{ borderTop: "1px solid var(--border)", background: i % 2 === 0 ? "transparent" : "rgba(100,116,139,0.03)" }}>
                                  <td style={{ padding: "12px 14px", fontWeight: 600, maxWidth: 160 }}>
                                    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                                      ...
                                      <i className="bi bi-globe" style={{ color: "var(--muted-foreground)", fontSize: 14 }} />}
                                      <span style={{ whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", display: "inline-block" }} title={c.name}>{c.name}</span>
                                    </div>
                                    <div style={{ fontSize: 10, color: "var(--muted-foreground)", marginTop: 4, fontWeight: 500 }}>{days.length} ngày báo cáo</div>
                                  </td>
                                  <td style={{ padding: "12px 14px" }}>
                                    <span style={{ display: "inline-flex", alignItems: "center", gap: 4, background: "var(--background)", border: `1px solid ${statusColor}30`, color: statusColor, padding: "2px 8px", borderRadius: 12, fontSize: 10, fontWeight: 800 }}>
                                      <div style={{ width: 6, height: 6, borderRadius: "50%", background: statusColor, animation: c.status === "ACTIVE" ? "status-pulse-dot 2s infinite" : "none" }} />
                                      {c.status}
                                    </span>
                                  </td>
                                  <td style={{ padding: "12px 14px", textAlign: "right", fontWeight: 700, fontFamily: "monospace", fontSize: 14 }}>
                                    {Math.round(totalSpend).toLocaleString()} đ
                                  </td>
                                </tr>
                              );
                            })}
                ...
                <div style={{ maxWidth: 800, margin: "0 auto" }}>
                  {activeTab === 'yt' ? (
                    ...
                    <>
                      ...
                      <p style={{ fontSize: 16, color: "var(--muted-foreground)", marginBottom: 48 }}>
                        Tài liệu này hướng dẫn chi tiết cách tạo dự án trên <b>TikTok For Business Developers</b> và cấu hình <b>OAuth2</b> để tự động đồng bộ hóa các chỉ số hiệu quả từ tài khoản quảng cáo TikTok của bạn.
                      </p>
                      ...
                      <section style={{ marginBottom: 48 }}>
                        ...
                        <ul style={{ listStyle: "none", padding: 0, display: "flex", flexDirection: "column", gap: 20 }}>
                          <li style={{ display: "flex", gap: 12 }}>
                            ...
                            <div>
                              <div style={{ fontWeight: 700, fontSize: 16 }}>Tài khoản Business Center (BC) TikTok Ads.</div>
                              <div style={{ fontSize: 13, color: "var(--muted-foreground)", marginTop: 4 }}>
                                Tài khoản này sở hữu và quản lý các tài khoản quảng cáo TikTok Ads của doanh nghiệp.
                              </div>
                            </div>
                          </li>
                        </ul>
                      ...
                      <section style={{ marginBottom: 48 }}>
                        ...
                        {[
                          {
                            step: "Bước 1",
                            title: "Đăng ký Tài khoản Nhà phát triển TikTok",
                            content: "Bạn cần có tài khoản Developer để tạo và quản lý ứng dụng tích hợp.",
                            subSteps: [
                              "Truy cập: https://developers.tiktok.com/ và đăng nhập.",
                              "Nhấn vào mục **Console** ở góc phải màn hình.",
                              "Đăng ký thông tin nhà phát triển (Doanh nghiệp hoặc Cá nhân) và chờ phê duyệt (thường mất 24h)."
                            ]
                          },
                          {
                            step: "Bước 2",
                            title: "Tạo Ứng dụng mới (Create App)",
                            content: "Ứng dụng sẽ hoạt động như một chiếc cầu kết nối bảo mật giữa CRM và TikTok.",
                            subSteps: [
                              "Trong Developer Console, nhấn **Create App**.",
                              "Chọn loại ứng dụng: **Marketing API**.",
                              "Điền thông tin App (Tên app, logo, website doanh nghiệp) và nhấn **Submit**."
                            ]
                          },
                          {
                            step: "Bước 3",
                            title: "Bật các quyền API cần thiết (Permissions)",
                            content: "Ứng dụng cần được cấp quyền đọc báo cáo quảng cáo của tài khoản.",
                            subSteps: [
                              "Truy cập vào ứng dụng của bạn, chọn mục **Permissions**.",
                              "Bật các quyền: **Ads Management** (để xem thông tin chiến dịch) và **Ads Reporting** (để lấy số liệu báo cáo).",
                              "Cấu hình **Redirect URI** ở phần App Settings (Sao chép link màu đỏ bên dưới)."
                            ],
                            details: [
                              { label: "Redirect URI (Copy dòng này)", value: redirectUri }
                            ]
                          ...
                          {
                            step: "Bước 4",
                            title: "Ủy quyền tài khoản quảng cáo",
                            content: "Liên kết tài khoản quảng cáo thực tế với CRM.",
                            subSteps: [
                              "Copy **App ID** và **Secret Key** trong phần App Credentials trên TikTok Console.",
                              "Quay lại form bên cạnh và dán vào, sau đó nhấn **Lưu cấu hình**.",
                              "Nhấn nút **Kết nối tài khoản TikTok** để chuyển sang trang đăng nhập ủy quyền của TikTok.",
                              "Chọn đúng tài khoản Business Center chứa các tài khoản quảng cáo và nhấn **Confirm**."
                            ]
                          }
                        ].map((item, idx) => (
                          <div key={idx} style={{ marginBottom: 60, paddingLeft: 20, borderLeft: "2px solid rgba(0,0,0,0.1)" }}>
                            ...
                            {item.details && (
                              <div style={{ marginTop: 20, display: "grid", gridTemplateColumns: "1fr", gap: 12 }}>
                                {item.details.map((d, i) => (
                                  <div key={i} style={{ padding: "16px 20px", background: "rgba(0,0,0,0.03)", borderRadius: 16, border: "1px dashed var(--foreground)" }}>
                                    <div style={{ fontSize: 11, fontWeight: 900, color: "var(--muted-foreground)", textTransform: "uppercase", marginBottom: 6 }}>{d.label}</div>
                    ...
                    <>
                      <h1 style={{ fontSize: 36, fontWeight: 900, marginBottom: 16, letterSpacing: "-0.03em" }}>HƯỚNG DẪN KẾT NỐI FACEBOOK LEAD ADS & WEBHOOKS</h1>
                      <p style={{ fontSize: 16, color: "var(--muted-foreground)", marginBottom: 48 }}>
                        Tài liệu này hướng dẫn chi tiết quy trình tự động đồng bộ hóa thông tin khách hàng từ các chiến dịch <b>Facebook Lead Ads</b> và <b>Instagram Leads</b> về hệ thống CRM thông qua nền tảng trung gian <b>Make.com</b>.
                      </p>
                      ...
                      <section style={{ marginBottom: 48 }}>
                        ...
                        <ul style={{ listStyle: "none", padding: 0, display: "flex", flexDirection: "column", gap: 20 }}>
                          <li style={{ display: "flex", gap: 12 }}>
                            ...
                            <div>
                              <div style={{ fontWeight: 700, fontSize: 16 }}>Quyền Quản trị viên (Admin) của Fanpage Facebook.</div>
                              <div style={{ fontSize: 13, color: "var(--muted-foreground)", marginTop: 4 }}>
                                Bạn cần có quyền quản lý trang và truy cập cài đặt tích hợp của trang đó để đọc dữ liệu Lead.
                              </div>
                            </div>
                          </li>
                          <li style={{ display: "flex", gap: 12 }}>
                            <i className="bi bi-check2-square" style={{ color: "#1877F2", fontSize: 18, marginTop: 2 }} />
                            <div>
                              <div style={{ fontWeight: 700, fontSize: 16 }}>Tài khoản Make.com (Hỗ trợ gói miễn phí).</div>
                              <div style={{ fontSize: 13, color: "var(--muted-foreground)", marginTop: 4 }}>
                                Make.com đóng vai trò như robot trung chuyển đẩy dữ liệu từ Facebook về CRM qua API.
                              </div>
                            </div>
                          </li>
                        </ul>
