...
function PlanMonthlyContent() {
  ...
  const employeeId = session?.user?.employeeId || session?.user?.id || "unknown";
  const employeeName = session?.user?.name || "Nhân viên Marketing";

  const loadPlan = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch(`/api/marketing/monthly-plan?year=${year}&month=${month}&employeeId=${employeeId}`);
      const data = await res.json();
      setPlan((data as MMonthlyPlan[])[0] || { id: "", year, month, status: "draft", tasks: [] } as any);
  ...
  useEffect(() => {
      ...
      if (planIdParam) {
        ...
        try {
          ...
          if (data && !data.error) {
            ...
            setYear(data.year);
            setMonth(data.month);
            // After plan is set, related data will be loaded via the other effects/dependencies
          }
        } finally { setLoading(false); }
      } else {
        loadPlan();
    ...
    init();
  }, [planIdParam, loadPlan, loadAnnualContents, loadCompanyInfo]);
  ...
  const ensurePlanId = async (): Promise<string | null> => {
    ...
    const res = await fetch("/api/marketing/monthly-plan", {
      ...
      body: JSON.stringify({ year, month, employeeId, employeeName: employeeName || "Nhân viên MKT" }),
    });
    if (res.ok) {
      const data = await res.json();
      setPlan(data);
  ...
  const handleChangeStatus = async (status: string) => {
    ...
    setSaving(true);
    try {
      await fetch(`/api/marketing/monthly-plan/${pId}`, {
  ...
  const handleUpdateProductCategories = async () => {
    ...
    setSaving(true);
    try {
      const prodsStr = Array.from(editingSanPham.selected).join(", ");
  ...
  const openEditTask = (t: MTask) => {
    setEditTask(t);
    setTaskForm({
      title: t.title, description: t.description || "",
      taskType: t.taskType || "", category: t.category || "",
      taskGroup: t.taskGroup || "",
      taskSubGroup: t.taskSubGroup || "",
      week1: t.week1, week2: t.week2, week3: t.week3, week4: t.week4,
      assigneeName: t.assigneeName || "", status: t.status, budget: t.budget || ""
    });
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            {plan && plan.status === "draft" && (plan?.tasks?.length || 0) > 0 && (
              <button onClick={() => handleChangeStatus("submitted")} disabled={saving}
                ...
                <i className="bi bi-send" /> Gửi duyệt
              </button>
            )}
            {plan && plan.status === "submitted" && (
              <button onClick={() => handleChangeStatus("draft")} disabled={saving}
                style={{ padding: "6px 14px", borderRadius: 8, border: "1px solid var(--border)", background: "var(--card)", color: "var(--foreground)", fontWeight: 700, fontSize: 12, cursor: "pointer", display: "flex", alignItems: "center", gap: 6, transition: "background 0.2s" }} onMouseEnter={e => e.currentTarget.style.background = "var(--muted)"} onMouseLeave={e => e.currentTarget.style.background = "var(--card)"}>
                <i className="bi bi-arrow-counterclockwise" /> Rút về nháp
              </button>
            )}
          </div>
        </div>
          ...
          <>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", marginBottom: 16, flexWrap: "wrap", gap: 16, padding: "0 4px" }}>
              <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                <div style={{ fontSize: 12, fontWeight: 700, textTransform: "uppercase", letterSpacing: 1, color: "var(--muted-foreground)" }}>Mục chi tiết</div>
                ...
                <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
                  ...
                  {selectedTaskIds.size > 0 && (
                    <button onClick={() => setConfirmBulkDel(true)}
                      style={{ padding: "8px 16px", borderRadius: 10, border: "none", background: "rgba(239, 68, 68, 0.1)", color: "#ef4444", fontWeight: 700, fontSize: 13, cursor: "pointer", display: "flex", alignItems: "center", gap: 8, transition: "background 0.2s" }} onMouseEnter={e => e.currentTarget.style.background = "rgba(239, 68, 68, 0.15)"} onMouseLeave={e => e.currentTarget.style.background = "rgba(239, 68, 68, 0.1)"}>
                      <i className="bi bi-trash" /> Xoá ({selectedTaskIds.size})
                    </button>
                  )}
                </div>
              )}
                ...
                <table style={{ width: "100%", borderCollapse: "collapse", tableLayout: "fixed" }}>
                  ...
                  <tbody>
                    {(() => {
                      ...
                      const ALL_GROUPS = [...TASK_GROUPS, ...customGroups];
                      return ALL_GROUPS.map((group, gIdx) => {
                        ...
                        const buildWeekCells = (t: MTask) => {
                          const checked = [t.week1, t.week2, t.week3, t.week4];
                          const cells: { active: boolean; span: number }[] = [];
                          let i = 0;
                          while (i < 4) {
                            if (checked[i]) {
                              let span = 1;
                              while (i + span < 4 && checked[i + span]) span++;
                              cells.push({ active: true, span });
                              i += span;
                            } else {
                        ...
                        return (
                          <React.Fragment key={group.value}>
                            ...
                            <tr
                              ...
                              onClick={(e) => {
                                // Mở/đóng group, loại trừ click vào checkbox
                            ...
                            >
                              ...
                              <td style={{ ...tdStyle, verticalAlign: "middle" }}>
                                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                                  <span style={{ display: "inline-flex", alignItems: "center", gap: 8, fontWeight: 800, fontSize: 12, color: group.color, textTransform: "uppercase", letterSpacing: "0.5px" }}>
                                  ...
                                  {canEdit && group.value !== "chu_de_thang" && (
                                    <button
                                      ...
                                      style={{ background: "transparent", border: "none", color: group.color, cursor: "pointer", padding: "2px 4px", display: "flex", alignItems: "center" }}>
                                      <i className="bi bi-plus-lg" style={{ fontSize: 14 }} />
                                    </button>
                                  )}
                                </div>
                              </td>
                              {/* Nếu có sub-groups thì header row không hiện content */}
                              {hasSubGroups
                                ? [1, 2, 3, 4].map(w => <td key={w} style={{ borderLeft: "1px solid var(--border)" }} />)
                                : (["week1Content", "week2Content", "week3Content", "week4Content"] as const).map((field, wi) => {
                                  const weekContents = groupTasks.map(t => t[field]).filter(Boolean);
                                  return (
                                    <td key={wi} style={{ ...tdStyle, borderLeft: "1px solid var(--border)", fontSize: 12, lineHeight: 1.4 }}>
                                      {weekContents.map((c, ci) => (
                                        <div key={ci} style={{ display: "flex", alignItems: "flex-start", gap: 4, marginBottom: ci < weekContents.length - 1 ? 2 : 0 }}>
                                          <span style={{ color: group.color, marginTop: 4, flexShrink: 0, fontSize: 8 }}>●</span>
                                          <span>{c}</span>
                                        </div>
                                      ))}
                                    </td>
                            ...
                            {/* Sub-group rows — ẩn khi collapsed */}
                            {hasSubGroups && !isCollapsed && group.subGroups.map((sub, sIdx) => {
                              const subTasks = plan?.tasks?.filter(t => t.taskGroup === group.value && t.taskSubGroup === sub.value) || [];
                                ...
                                <React.Fragment key={sub.value}>
                                      ...
                                      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                                        ...
                                        {canEdit && sub.value !== "san_pham" && (
                                          <button
                                            ...
                                            style={{ background: "transparent", border: "none", color: group.color, cursor: "pointer", padding: "2px 6px" }}>
                                            <i className="bi bi-plus-circle-fill" style={{ fontSize: 13 }} />
                                          </button>
                                        )}
                                      </div>
                                    </td>
                                  ...
                                  {(() => {
                                    ...
                                    return packedRows.slice(1).map((row, rIdx) => {
                                      ...
                                      return (
                                        <tr key={`row-${rIdx}`} style={{ background: `${group.color}06` }}>
                                          <td style={{ ...tdStyle, textAlign: "center", verticalAlign: "middle" }}>
                                            <input
                                              title="Chọn để xóa"
                                              type="checkbox"
                                              ...
                                              onChange={(e) => {
                                                row.forEach(t => toggleTaskSelection(t.id, e.target.checked));
                                              }}
                                          ...
                                          {cells.map((cell, ci) => cell.active && cell.task ? (
                                                  ...
                                                  <button
                                                    onClick={() => {
                                                      ...
                                                      setTaskForm({
                                                        ...
                                                        taskGroup: child.taskGroup || "",
                                                        taskSubGroup: child.taskSubGroup || "",
                                                        assigneeName: child.assigneeName || "",
                                                        week1: child.week1, week2: child.week2, week3: child.week3, week4: child.week4,
                                            ...
                                            <td key={ci} style={{ ...tdStyle, borderTop: "1px dashed var(--border)", borderLeft: "1px solid var(--border)" }}></td>
                                          ))}
                                        </tr>
                                      );
                                    });
                            ...
                            {group.value !== "chu_de_thang" && !isCollapsed && (plan?.tasks || []).filter(t => t.taskGroup === group.value && !t.taskSubGroup).map(t => {
                                      ...
                                      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                                        ...
                                        {canEdit && (
                                          <button
                                            ...
                                            style={{ border: "none", background: "transparent", color: group.color, cursor: "pointer", padding: "2px 6px" }}>
                                            <i className="bi bi-plus-circle-fill" style={{ fontSize: 13 }} />
                                          </button>
                                        )}
                                      </div>
                                        ...
                                        <tr key={`dyn-row-${rIdx}`} style={{ background: `${group.color}06` }}>
                                          <td style={{ ...tdStyle, textAlign: "center", verticalAlign: "middle" }}>
                                            <input
                                              title="Chọn để xóa"
                                              type="checkbox"
                                              checked={row.every(child => selectedTaskIds.has(child.id))}
                                              onChange={(e) => {
                                                row.forEach(child => toggleTaskSelection(child.id, e.target.checked));
                                              }}
                                              style={{ width: 14, height: 14, cursor: "pointer", opacity: 0.8 }}
                                            />
                                          </td>
                  ...
                  <tfoot style={{ position: "sticky", bottom: 0, zIndex: 20, background: "var(--card)", boxShadow: "0 -2px 10px rgba(0,0,0,0.02)" }}>
                    <tr style={{ background: "rgba(0,48,135,0.04)", borderTop: "2px solid var(--border)" }}>
                      ...
                      {[1, 2, 3, 4].map(w => {
                        const field = `week${w}Content` as keyof MTask;
            ...
            <motion.div initial={{ x: "100%" }} animate={{ x: 0 }} exit={{ x: "100%" }} transition={{ type: "spring", damping: 28, stiffness: 220 }}
              ...
              <div style={{ padding: "16px 24px", display: "flex", justifyContent: "space-between", alignItems: "center", borderBottom: "1px solid var(--border)", background: "var(--card)" }}>
                <div style={{ fontSize: 12, fontWeight: 700, color: "var(--muted-foreground)", letterSpacing: 0.5 }}>
                  {editTask ? "CHỈNH SỬA CÔNG VIỆC" : "THÊM CÔNG VIỆC MỚI"}
                </div>
                ...
                <tbody>
                  {(() => {
                    const customGroups = (plan?.tasks || [])
                      .filter((t: any) => t.taskType === "group" && !t.taskGroup)
                      .map((t: any) => ({
                        value: t.id, label: t.title, color: "#C9A84C", icon: "bi-layers", subGroups: [] as any[], isCustom: true
                      }));
                    const ALL_GROUPS = [...TASK_GROUPS, ...customGroups];
