...
export default function CompetitorsPage() {
      ...
      if (data.length === 0) {
        ...
        try {
          ...
          if (localData.length > 0) {
            ...
            const freshData = await getCompetitors();
  ...
  async function callScanApi(url: string): Promise<Competitor | null> {
    ...
    return data.data as Competitor;
  }
                    ...
                    <div className="app-card" style={{ padding: 24, borderRadius: 16 }}>
                      ...
                      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 16, marginBottom: 24 }}>
                        <div style={{ padding: "16px 20px", background: "white", border: "1px solid #eef2f6", borderRadius: 14 }}>
                          ...
                          <div style={{ fontSize: 20, fontWeight: 900 }}>{selectedComp.metrics.adsCount} <span style={{ fontSize: 12, color: "#10b981", fontWeight: 700 }}>Đang chạy</span></div>
                        </div>
                        <div style={{ padding: "16px 20px", background: "white", border: "1px solid #eef2f6", borderRadius: 14 }}>
                          <div style={{ fontSize: 11, color: "var(--muted-foreground)", fontWeight: 700, textTransform: "uppercase", marginBottom: 8, display: "flex", alignItems: "center", gap: 6 }}>
                            <i className="bi bi-chat-heart" style={{ color: "#ec4899" }} /> Thảo luận MXH
                          </div>
                          <div style={{ fontSize: 20, fontWeight: 900 }}>{selectedComp.metrics.socialMentions.toLocaleString("vi-VN")}</div>
                        </div>
                        <div style={{ padding: "16px 20px", background: "white", border: "1px solid #eef2f6", borderRadius: 14 }}>
                          <div style={{ fontSize: 11, color: "var(--muted-foreground)", fontWeight: 700, textTransform: "uppercase", marginBottom: 8, display: "flex", alignItems: "center", gap: 6 }}>
                            <i className="bi bi-currency-dollar" style={{ color: "#f59e0b" }} /> Ước tính Ngân sách
                          </div>
                          <div style={{ fontSize: 20, fontWeight: 900 }}>~{selectedComp.metrics.estBudget}</div>
                        </div>
                      </div>
                      ...
                      {selectedComp.strategySuggestions && (
                        <div style={{ marginBottom: 32 }}>
                          ...
                          <div style={{ padding: "20px 24px", background: "linear-gradient(135deg, #1e1b4b, #312e81)", borderRadius: 16, color: "white", boxShadow: "0 10px 20px -5px rgba(30, 27, 75, 0.2)" }}>
                            ...
                            <div style={{ fontSize: 14, lineHeight: 1.6, fontWeight: 500 }}>{selectedComp.strategySuggestions.tacticalAdvice}</div>
                          </div>
                        </div>
                      )}
                      ...
                      <div style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 24 }}>
                        {[
                          ...
                          { 
                            ...
                            status: selectedComp.metrics.adsCount > 0 ? `${selectedComp.metrics.adsCount} Ads đang chạy` : "Chờ đồng bộ Apify...",
                            active: selectedComp.metrics.adsCount > 0,
                            details: selectedComp.metrics.adsCount > 0 ? [
                          ...
                          { 
                            ...
                            status: selectedComp.metrics.socialMentions > 0 ? `${selectedComp.metrics.socialMentions.toLocaleString("vi-VN")} kết quả` : "Không có dữ liệu",
                            active: selectedComp.metrics.socialMentions > 0,
                            details: [
