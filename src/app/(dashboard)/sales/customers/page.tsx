              ...
              <div className="modal-body p-4 bg-white" style={{ overflowY: "auto" }}>
                <div className="container-fluid">
                  <div className="row g-4">
                    {/* Cột trái: Thông tin đại lý, Doanh số & Chính sách */}
                    <div className="col-12 col-lg-4 d-flex flex-column gap-4">
                      ...
                      <div>
                        ...
                        <div className="d-flex flex-column bg-light rounded-4 border border-light" style={{ fontSize: "14px", padding: "20px", gap: "10px" }}>
                          ...
                          {selectedCustomer.ghiChu && (
                            <div className="mt-3 pt-3 border-top" style={{ borderColor: "rgba(0,0,0,0.05)" }}>
                              <span className="fw-bold text-secondary text-uppercase d-block mb-2" style={{ fontSize: "11px", letterSpacing: "0.05em" }}>
                                Ghi chú
                              </span>
                              <p className="text-dark bg-white p-3 rounded-3 mb-0 border shadow-sm" style={{ fontSize: "13.5px", whiteSpace: "pre-line" }}>
                                {selectedCustomer.ghiChu}
                              </p>
                            </div>
                          )}
                      ...
                      <div>
                        ...
                        <div className="row g-3">
                          <div className="col-12">
                            <div className="p-4 bg-success bg-opacity-10 rounded-4 border border-success border-opacity-25 h-100">
                              <span className="text-success d-block mb-1 fw-medium" style={{ fontSize: "13px" }}>Doanh số thực tế năm nay</span>
                              <strong className="text-success d-block" style={{ fontSize: "24px" }}>
                                {((selectedCustomer as any).yearlySales ?? 0).toLocaleString("vi-VN")} ₫
                              </strong>
                            </div>
                          </div>
                          <div className="col-12">
                            <div className="p-4 bg-primary bg-opacity-10 rounded-4 border border-primary border-opacity-25 h-100">
                              <span className="text-primary d-block mb-1 fw-medium" style={{ fontSize: "13px" }}>Doanh số cam kết năm</span>
                              <strong className="text-primary d-block" style={{ fontSize: "24px" }}>
                      ...
                      <div>
                        <span className="fw-bold text-secondary text-uppercase d-block mb-3" style={{ fontSize: "12px", letterSpacing: "0.05em" }}>
                          Chính sách thưởng & Chiết khấu (Theo biên bản)
                        </span>
                        <div className="row g-3">
                          {(() => {
                            let parsedForm = null;
                            try {
                              parsedForm = JSON.parse(selectedCustomer.formValues || "{}");
                            } catch (e) {}
                            const bbBonuses = parsedForm?.bbBonuses || [];

                            if (bbBonuses.length === 0) return (
                              <div className="col-12">
                                <div className="text-muted fst-italic py-4 text-center bg-light rounded-4 border">
                                  <i className="bi bi-inbox fs-3 d-block mb-2 text-secondary"></i>
                                  Chưa có chính sách thưởng và chiết khấu nào được thiết lập.
                                </div>
                              </div>
                            );

                            return bbBonuses.map((bonus: any, idx: number) => {
                              const icons = ["bi-calendar-event text-success", "bi-trophy text-warning", "bi-graph-up-arrow text-danger", "bi-star text-primary", "bi-gift text-info"];
                              const iconClass = icons[idx % icons.length];
                              
                              return (
                                <div className="col-12" key={idx}>
                                  <div className="p-4 bg-light rounded-4 border border-light h-100 d-flex flex-column shadow-sm transition-hover">
                                    <div className="d-flex align-items-center gap-3 mb-3">
                                      <div className="bg-white rounded-circle d-flex align-items-center justify-content-center shadow-sm flex-shrink-0" style={{ width: "48px", height: "48px" }}>
                                        <i className={`bi ${iconClass} fs-4`} />
                                      </div>
                                      <h6 className="fw-bold text-dark mb-0 fs-6 lh-base">{bonus.title}</h6>
                                    </div>
                                    <p className="text-muted flex-grow-1" style={{ fontSize: "13.5px", lineHeight: "1.5" }}>
                                      {bonus.desc}
                                    </p>
                                    <div className="mt-3 p-3 rounded-3 bg-white border shadow-sm">
                                      <span className="text-secondary fw-semibold d-block mb-1" style={{ fontSize: "12px" }}>Công thức tính:</span>
                                      <span className="text-dark fw-bold font-monospace" style={{ fontSize: "13px", wordBreak: "break-word" }}>
                                        {bonus.formula}
                                      </span>
                                    </div>
                                  </div>
                    ...
                    {/* Cột phải: Dành cho nội dung thiết kế tương lai */}
                    <div className="col-12 col-lg-8">
                      <div className="h-100 bg-light rounded-4 border border-light d-flex flex-column align-items-center justify-content-center text-muted fst-italic p-5 text-center min-vh-50">
                        <i className="bi bi-layout-text-window-reverse fs-1 mb-3 text-secondary opacity-50"></i>
                        <h5 className="fw-medium text-dark opacity-75">Khu vực thông tin mở rộng</h5>
                        <p className="mb-0 fs-6">Phần không gian này sẽ được thiết kế bổ sung thêm các tính năng và dữ liệu chi tiết khác của đại lý.</p>
                      </div>
                    </div>
                  </div>
                </div>
