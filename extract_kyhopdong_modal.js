...
const modalStartStr = `{/* ── Ky Hop Dong Modal ── */}`;
const modalEndStr = `{/* ── Ky Bien Ban Modal ── */}`;

const startIndex = pageContent.indexOf(modalStartStr);
const endIndex = pageContent.indexOf(modalEndStr);
...
const modalJSX = pageContent.substring(startIndex, endIndex);

// We will write the KyHopDongModal.tsx manually since the JSX requires minor changes.
...
fs.writeFileSync('src/components/sales/partners/KyHopDongModal.tsx', modalComponentCode);

// Replace the JSX block in the original file
const replacement = `      <KyHopDongModal 
        isOpen={showKyHopDongModal}
        onClose={() => setShowKyHopDongModal(false)}
        ...
        onSuccess={(updatedPartner) => {
          ...
          toastSuccess("Thành công", "Đã cập nhật thông tin ký hợp đồng thành công!");
        }}
      />\n\n`;

pageContent = pageContent.substring(0, startIndex) + replacement + pageContent.substring(endIndex);

// I also need to add the import to the top of pageContent
if (!pageContent.includes('KyHopDongModal')) {
  pageContent = pageContent.replace('import React, { useState, useMemo, useEffect } from "react";', 'import React, { useState, useMemo, useEffect } from "react";\nimport { KyHopDongModal } from "@/components/sales/partners/KyHopDongModal";');
}

fs.writeFileSync(pageFile, pageContent);
console.log("Extracted KyHopDongModal!");
